...
const addTask = async (req, res) => {
  ...
  const { name, description, startDate, endDate, completed } = req.body; // agarro los parametros que envie en el body
  await model.Task.create({ name, description, completed, startDate, endDate, UserId: userId }).then(
    async (createdTask) => {
...
const addTaskOpcionA = async (req, res) => {
  const { userId } = req;
  //return res.status(201).json({ message : "userid a ingresar" + userId });
